# SimpsonsCharacter Lua SDK

The Lua SDK for the SimpsonsCharacter API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Character(nil):list(nil, nil)
...
```lua
local result, err = client:Character(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:SimpsonsCharacter(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── simpsons-character_sdk.lua    -- Main SDK module
...
The main module (`simpsons-character_sdk`) exports the SDK constructor
