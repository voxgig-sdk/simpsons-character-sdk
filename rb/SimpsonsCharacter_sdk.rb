...
require_relative 'SimpsonsCharacter_types'
...
class SimpsonsCharacterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SimpsonsCharacterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SimpsonsCharacterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SimpsonsCharacterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SimpsonsCharacterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SimpsonsCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SimpsonsCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SimpsonsCharacterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SimpsonsCharacterError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue SimpsonsCharacterError => err
    ...
    if fetched.is_a?(Hash)
      status = SimpsonsCharacterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SimpsonsCharacterError.new(
        "graphql_error", "SimpsonsCharacterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SimpsonsCharacterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
