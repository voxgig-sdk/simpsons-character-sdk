...
class LocationEntityTest < Minitest::Test
  def test_create_instance
    testsdk = SimpsonsCharacterSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = SimpsonsCharacterSDK.test(seed, nil)
    seen = base.Location(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = SimpsonsCharacterConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = SimpsonsCharacterSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SIMPSONSCHARACTER_TEST_LOCATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def location_basic_setup(extra)
  ...
  client = SimpsonsCharacterSDK.test(options, extra)
  ...
  entid_env_raw = ENV["SIMPSONSCHARACTER_TEST_LOCATION_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "SIMPSONSCHARACTER_TEST_LOCATION_ENTID" => idmap,
    "SIMPSONSCHARACTER_TEST_LIVE" => "FALSE",
    "SIMPSONSCHARACTER_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["SIMPSONSCHARACTER_TEST_LOCATION_ENTID"])
