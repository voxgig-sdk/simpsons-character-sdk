...
import (
	...
	omni "github.com/voxgig-sdk/simpsons-character-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SimpsonsCharacterError); is {
				out := map[string]any{
					"name":    "SimpsonsCharacterError",
...
func omniCtx(arg any, client *sdk.SimpsonsCharacterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SimpsonsCharacterSDK); is {
			client = live
