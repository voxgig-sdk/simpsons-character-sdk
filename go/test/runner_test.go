...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SIMPSONSCHARACTER_TEST_LIVE") == "TRUE" ||
		os.Getenv("SIMPSONSCHARACTER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SIMPSONSCHARACTER_TEST_EXPLAIN"); explain != "" {
