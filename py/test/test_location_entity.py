...
class TestLocationEntity:
    ...
    def test_should_stream(self):
        ...
        base = SimpsonsCharacterSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SIMPSONS_CHARACTER_TEST_LOCATION_ENTID JSON to run live")
        client = setup["client"]
...
def _location_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "SIMPSONS_CHARACTER_TEST_LOCATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SIMPSONS_CHARACTER_TEST_LOCATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("SIMPSONS_CHARACTER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = SimpsonsCharacterSDK(helpers.to_map(merged_opts))

    _live = env.get("SIMPSONS_CHARACTER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("SIMPSONS_CHARACTER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
