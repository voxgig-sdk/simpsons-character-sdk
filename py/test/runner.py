# SimpsonsCharacter SDK test runner
...
class SimpsonsCharacterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SimpsonsCharacterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SimpsonsCharacterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SimpsonsCharacterTestRunner.getenv("SIMPSONS_CHARACTER_TEST_LIVE")
        override = SimpsonsCharacterTestRunner.getenv("SIMPSONS_CHARACTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SimpsonsCharacterTestRunner.getenv(key)
        ...
        explain = SimpsonsCharacterTestRunner.getenv("SIMPSONS_CHARACTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SimpsonsCharacterTestRunner._test_control is not None:
            return SimpsonsCharacterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SimpsonsCharacterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SimpsonsCharacterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SimpsonsCharacterTestRunner.load_test_control()
...
def load_env_local():
    SimpsonsCharacterTestRunner.load_env_local()


def env_override(m):
    return SimpsonsCharacterTestRunner.env_override(m)


def entity_data(v):
    return SimpsonsCharacterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SimpsonsCharacterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SimpsonsCharacterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SimpsonsCharacterTestRunner.load_test_control()


def live_client_options():
    return SimpsonsCharacterTestRunner.live_client_options()


def live_delay_ms():
    return SimpsonsCharacterTestRunner.live_delay_ms()
