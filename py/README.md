# SimpsonsCharacter Python SDK

The Python SDK for the SimpsonsCharacter API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Character(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Character(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.SimpsonsCharacter(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── simpsonscharacter_sdk.py         -- Main SDK module
