...
class SimpsonsCharacterMakeError
{
    public static function call(?SimpsonsCharacterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SimpsonsCharacterContext([], null);
        }
        $op = $ctx->op ?? new SimpsonsCharacterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SimpsonsCharacterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SimpsonsCharacterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new SimpsonsCharacterError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof SimpsonsCharacterError) {
