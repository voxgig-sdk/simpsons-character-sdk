...
// SimpsonsCharacter SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SimpsonsCharacterTestFeature extends SimpsonsCharacterBaseFeature
{
    ...
    public function init(SimpsonsCharacterContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SimpsonsCharacterContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SimpsonsCharacterParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SimpsonsCharacterContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SimpsonsCharacterContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SimpsonsCharacterParam::call($ctx, $k);
