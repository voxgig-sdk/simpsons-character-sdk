...
//
// NewSimpsonsCharacterSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.SimpsonsCharacterSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewSimpsonsCharacterSDK(opts)
}
...
func fcCandidates(client *sdk.SimpsonsCharacterSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.SimpsonsCharacterSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.SimpsonsCharacterSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.SimpsonsCharacterError
