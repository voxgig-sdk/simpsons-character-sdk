...
module SimpsonsCharacterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SimpsonsCharacterResponse.new({})
    ...
    if fetch_err
      ...
      response = SimpsonsCharacterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SimpsonsCharacterResponse.new(fetched)
    else
