# simpsons-character-cli

boru-driven command-line client **and** interactive REPL for the SimpsonsCharacter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/simpsons-character-cli)
make build

# 2. See usage (words, entities, env vars)
./simpsons-character-cli --help

# 3. Provide credentials once, via the environment
export SIMPSONS_CHARACTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./simpsons-character-cli list character
./simpsons-character-cli load 1 character            # {id:1} shorthand
./simpsons-character-cli load '{id:1}' character       # explicit match map
./simpsons-character-cli list episode

# 5. Override the API base URL for a single call
SIMPSONS_CHARACTER_BASE=https://api.example.com ./simpsons-character-cli list character

# 6. No arguments -> interactive REPL
./simpsons-character-cli
simpsons-character> list character
simpsons-character> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/simpsons-character-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/simpsons-character-cli list character
   ```
...
```sh
./simpsons-character-cli load 1 character          # scalar shorthand for {id:1}
./simpsons-character-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `simpsons-character>`). Each line is
evaluated as its own boru expression:

```text
$ ./simpsons-character-cli
simpsons-character> list character
simpsons-character> /help
simpsons-character> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `character`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/simpsons-character-cli`. |
...
(`./simpsons-character-cli <expr>`) and interactively (the REPL), and expressions compose the
