...
// SimpsonsCharacter SDK retry feature
...
class SimpsonsCharacterRetryFeature extends SimpsonsCharacterBaseFeature
{
    ...
    public function init(SimpsonsCharacterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SimpsonsCharacterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SimpsonsCharacterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SimpsonsCharacterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
