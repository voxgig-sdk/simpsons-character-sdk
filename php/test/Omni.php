...
//    only - a typed SimpsonsCharacterContext there would make every ctx
...
final class SimpsonsCharacterOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SimpsonsCharacterContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SimpsonsCharacterSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SimpsonsCharacterResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SimpsonsCharacterError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SimpsonsCharacterResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SimpsonsCharacterError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SimpsonsCharacterError(is_string($code) ? $code : '', $msg);
    }
