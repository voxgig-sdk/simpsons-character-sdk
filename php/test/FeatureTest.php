...
// SimpsonsCharacter SDK feature test
//
...
class FtCtrl extends SimpsonsCharacterControl
{
...
class FtClient
{
    ...
    private SimpsonsCharacterUtility $utility;

    public function __construct(SimpsonsCharacterUtility $utility, array $options)
    {
    ...
    public function get_utility(): SimpsonsCharacterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SimpsonsCharacterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SimpsonsCharacterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new SimpsonsCharacterUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (SimpsonsCharacterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SimpsonsCharacterConfig::make_feature($name);
    ...
    private static function buildUrl(SimpsonsCharacterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SimpsonsCharacterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SimpsonsCharacterResponse($fetched);
        }
    ...
    private function fail(SimpsonsCharacterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SimpsonsCharacterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
