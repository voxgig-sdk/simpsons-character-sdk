# simpsons-character-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SimpsonsCharacter SDK as
two agent tools — `simpsons-character_list` and `simpsons-character_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/simpsons-character-mcp)
make build

# 2. Provide credentials via the environment
export SIMPSONS_CHARACTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user simpsons-character \
  -- /absolute/path/to/simpsons-character-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./simpsons-character-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// simpsons-character_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// simpsons-character_load: one record by id
{ "entity": "character", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/simpsons-character-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user simpsons-character \
     -- "$PWD"/dist/*/simpsons-character-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `simpsons-character_list` and `simpsons-character_load` tools now appear
   in new sessions. Ask the agent to *"list character using simpsons-character"*
   and it calls `simpsons-character_list` with `{"entity":"character"}`.
...
```sh
export SIMPSONS_CHARACTER_APIKEY=sk_live_xxx            # API key
export SIMPSONS_CHARACTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `simpsons-character_list` tool
...
### Call the `simpsons-character_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `simpsons-character_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `simpsons-character_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SIMPSONS_CHARACTER_APIKEY` | API key sent with every request. |
| `SIMPSONS_CHARACTER_BASE` | Optional override of the API base URL. |
...
```sh
./simpsons-character-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"simpsons-character_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
