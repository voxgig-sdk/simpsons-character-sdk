...
//     (\`new SimpsonsCharacterSDK(...)\` / \`SimpsonsCharacterSDK.test(...)\` become
//     \`SimpsonsCharacterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SimpsonsCharacterSDK'
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SimpsonsCharacterSDK(...)\` and any
// \`SimpsonsCharacterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SimpsonsCharacterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SimpsonsCharacterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
