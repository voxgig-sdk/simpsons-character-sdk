...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.SIMPSONSCHARACTER_TEST_LIVE ||
    'TRUE' === process.env.SIMPSONSCHARACTER_TEST_OVERRIDE
  ) {
  ...
  m.SIMPSONSCHARACTER_TEST_EXPLAIN = process.env.SIMPSONSCHARACTER_TEST_EXPLAIN || m.SIMPSONSCHARACTER_TEST_EXPLAIN
